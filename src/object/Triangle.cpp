...
void
Triangle::compute_normal(const bool reverse_normal) {
	normal = (v1 - v0) ^ (v2 - v0);
	normal.normalize();
...
#if 0 // add this for the case that ray hits inside of open rotational sweeping surface directly.
    float 		ndotr 	= sr.normal * ray.d;
    if ((ndotr > 0) && (ray.depth == 0)) {
        sr.normal = -normal;
    }
#endif // 1
