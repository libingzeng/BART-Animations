...
a(_a),
b(_b),
map_unit_a(abs(_a.x - _p0.x)),
map_unit_b(abs(_b.z - _p0.z)),
...
// ----------------------------------------------------------------  constructor
// this constructs with texture mapping unit

Rectangle::Rectangle(const Point3D& _p0, const Vector3D& _a, const Vector3D& _b, const float map_unit_a, const float map_unit_b)
:    GeometricObject(),
p0(_p0),
a(_a),
b(_b),
map_unit_a(map_unit_a),
map_unit_b(map_unit_b),
a_len_squared(a.len_squared()),
b_len_squared(b.len_squared()),
area(a.length() * b.length()),
inv_area(1.0 / area),
...
Rectangle::hit(const Ray& ray, double& tmin, ShadeRec& sr) const {
	...
	sr.u = (abs(p.x - p0.x) / map_unit_a) - floor((abs(p.x - p0.x) / map_unit_a)); // u,v just for retangle with y=c
	sr.v = (abs(p.z - p0.z) / map_unit_b) - floor((abs(p.z - p0.z) / map_unit_b)); // zlb modifies 2018051701
//    sr.u = p.x / map_unit_a;
//    sr.v = p.z / map_unit_b;
    if(isnan(sr.u) || isnan(sr.v))
    {
        sr.u = 0;
        sr.v = 0;
    }
