...
MeshTriangle::MeshTriangle(shared_ptr<Mesh> _mesh_ptr, const int i0, const int i1, const int i2)
	: 	GeometricObject(),
		mesh_ptr(_mesh_ptr),
...
MeshTriangle::compute_normal(const bool reverse_normal) {
    ...
    if(isnan(normal.x)){//zlb adds. 20180530
        //when three (bad) vertices don't make a triangle, "normal" will be NaN.
...
// ---------------------------------------------------------------- get_su
//sometimes, the range of (u,v) is not [0, 1], here get the scale of u
float
MeshTriangle::get_su(void) const{
    return (fmax(fmax(fabs(mesh_ptr->u[index0] - mesh_ptr->u[index1]),
                        fabs(mesh_ptr->u[index0] - mesh_ptr->u[index2])),
                  fabs(mesh_ptr->u[index1] - mesh_ptr->u[index2])));
}


// ---------------------------------------------------------------- get_sv
//sometimes, the range of (u,v) is not [0, 1], here get the scale of v
float
MeshTriangle::get_sv(void) const{
    return (fmax(fmax(fabs(mesh_ptr->v[index0] - mesh_ptr->v[index1]),
                        fabs(mesh_ptr->v[index0] - mesh_ptr->v[index2])),
                  fabs(mesh_ptr->v[index1] - mesh_ptr->v[index2])));
}
#endif//MIPMAP_ENABLE
