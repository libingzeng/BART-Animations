...
Instance::~Instance(void) {
#if 0 // zlb marks this for 2018051801
	if (object_ptr) {
		delete object_ptr;
		object_ptr = NULL;
	}
...
void
Instance::set_object(shared_ptr<GeometricObject> obj_ptr) {
	object_ptr = obj_ptr;
}
...
void
Instance::set_material(shared_ptr<Material> m_ptr) {
	material_ptr = m_ptr;
}
...
#if BUG3_FIXED//bug3: all robots have no texture
    inv_ray.rxOrigin = inv_matrix * inv_ray.rxOrigin;
    inv_ray.rxDirection = inv_matrix * inv_ray.rxDirection;
    inv_ray.ryOrigin = inv_matrix * inv_ray.ryOrigin;
    inv_ray.ryDirection = inv_matrix * inv_ray.ryDirection;
#endif//BUG3_FIXED
...
bool
Instance::shadow_hit(const Ray& ray, double& t) const {
	Ray inv_ray(ray);
	inv_ray.o = inv_matrix * inv_ray.o;
	inv_ray.d = inv_matrix * inv_ray.d;
...
Instance::scale(const double s) {
	...
	scaling_matrix.m[0][0] = s;
	scaling_matrix.m[1][1] = s;
	scaling_matrix.m[2][2] = s;
...
Instance::rotate_z(const double theta) {
	...
	Matrix inv_z_rotation_matrix;					// temporary inverse rotation matrix about z axis
...
Instance::rotate_axis(const double theta, const Vector3D& axis) {
    ...
    Matrix inv_axis_rotation_matrix;                    // temporary inverse rotation matrix about z axis
