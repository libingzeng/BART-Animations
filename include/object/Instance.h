...
class Instance: public GeometricObject {
	public:
		...
		Instance(shared_ptr<GeometricObject> obj_ptr);
		...
		void
		set_object(shared_ptr<GeometricObject> obj_ptr);
		...
		virtual shared_ptr<Material>
		get_material(void) const;

		virtual void
		set_material(shared_ptr<Material> materialPtr);
        ...
        virtual void
        rotate_axis(const double theta, const Vector3D& axis);
	...
	private:
		...
		shared_ptr<GeometricObject>	object_ptr;				// object to be transformed
