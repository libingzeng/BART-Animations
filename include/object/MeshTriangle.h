...
#include "Mesh.h"
#include <math.h> //zlb adds for using isnan(). 20180530

class MeshTriangle: public GeometricObject {	
	public:

		shared_ptr<Mesh> 		mesh_ptr;					// stores all the data
	...
	public:
		...
		MeshTriangle(shared_ptr<Mesh> _mesh_ptr, const int i1, const int i2, const int i3);		
...
#if MIPMAP_ENABLE
        float
        get_su(void) const;//sometimes, the range of (u,v) is not [0, 1], here get the scale of u
    
        float
        get_sv(void) const;//sometimes, the range of (u,v) is not [0, 1], here get the scale of u
#endif//MIPMAP_ENABLE
